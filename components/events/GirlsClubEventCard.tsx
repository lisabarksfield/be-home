...
export function GirlsClubEventCard({ event }: { event: EventEntry }) {
    ...
    <div className="rounded-2xl overflow-hidden flex flex-col" style={{ backgroundColor: GC.cream }}>
      <div className="p-6 flex flex-col flex-1">
        <div className="mb-3">
          ...
          <h3 className="text-xl leading-snug mt-2" style={displayHeadingStyle(GC.orangeDeep)}>
            {event.title}
          </h3>
          <p className="text-sm mt-1" style={{ color: "var(--color-charcoal)" }}>
            with {keepNamesTogether(event.practitioner)}
          </p>
          <p className={`${rounded.className} text-lg mt-2 font-medium`} style={{ color: GC.orangeDeep }}>
            {event.price}
          </p>
        </div>
        ...
        <div className={`space-y-2 mb-4 ${expanded ? "" : "line-clamp-4"}`}>
          {paragraphs.map((para, i) => (
            <p key={i} className="text-sm leading-relaxed" style={{ color: "var(--color-charcoal)" }}>
              {para}
            </p>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-auto">
          {"whatsappUrl" in event && event.whatsappUrl && (
            <a
              ...
              className={`${rounded.className} inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium`}
          ...
          {"bookingUrl2" in event && event.bookingUrl2 && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`${rounded.className} inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium hover:opacity-70 transition-opacity`}
              style={{ border: `1.5px solid ${GC.orangeDeep}`, color: GC.orangeDeep }}
            >
