...
export default function StudioBookingPage() {
  ...
  const fetchRentals = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/rentals?spaceId=studio&from=${rangeFrom}&to=${rangeTo}`
      );
  ...
  useEffect(() => {
    ...
    fetch(`/api/rentals?spaceId=studio&from=${startISO}&to=${endISO}`)
      .then((r) => r.json())
  ...
  return (
    <div>
      ...
      {success && (
        ...
        >
          <strong>Booking confirmed!</strong> Your studio booking has been
          submitted and will appear in the availability grid above. It is also
          now visible on the public Upcoming Events page.
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="max-w-lg space-y-5">
          ...
          <div>
            <label
              ...
              style={{ color: "var(--color-stone-deep)" }}
            >
              Spots available <span style={{ color: "var(--color-charcoal)" }}>*</span>
            </label>
            <input
              required
              type="number"
              ...
              onChange={(e) =>
                setForm({ ...form, spotsAvailable: e.target.value })
              }
              placeholder="e.g. 12"
              className="w-full px-4 py-2.5 rounded-lg text-sm outline-none"
              style={{
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Equipment setup, early access, special requirements…"
